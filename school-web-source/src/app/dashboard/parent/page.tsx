...
export default function ParentDashboard() {
  ...
  return (
    <div className="space-y-6">
      {/* Header Profile / Warning Banner */}
      {!child ? (
        <motion.div
          initial={{ opacity: 0, y: 16 }}
          animate={{ opacity: 1, y: 0 }}
          className="bg-cream border border-beige/40 rounded-3xl p-6 shadow-soft flex gap-4 items-start"
        >
          <div className="w-12 h-12 rounded-2xl bg-amber-100 flex items-center justify-center text-amber-700 shrink-0">
            <AlertCircle className="w-6 h-6" />
          </div>
          <div>
            <h2 className="text-lg font-display font-bold text-olive">No Student Linked</h2>
            <p className="text-sm text-olive/60 font-body mt-1 max-w-xl">
              Your parent account is not currently linked to any student profile. Please contact the administrator at <span className="font-semibold">sehrawatsonia27@gmail.com</span> with your child&apos;s name and admission number to request access.
            </p>
          </div>
        </motion.div>
      ) : (
      ...
      {/* Two column layout */}
      <div className={child ? "grid lg:grid-cols-2 gap-6" : "grid grid-cols-1 gap-6"}>
        {/* Announcements */}
        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}
          ...
          <div className="space-y-3">
            {announcements.length === 0 ? (
              <p className="text-sm text-olive/40 font-body py-4 text-center">No announcements posted yet.</p>
            ) : (
              announcements.map((a) => (
        ...
        {/* Teacher Notes (only if child is linked) */}
        {child && (
          <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.25 }}
            ...
            <div className="space-y-3">
              {notes.length === 0 ? (
                <p className="text-sm text-olive/40 font-body py-4 text-center">No notes from teacher yet.</p>
              ) : (
                notes.map((n) => (
      ...
      <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }}
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-3">
          {events.length === 0 ? (
            <p className="text-sm text-olive/40 font-body py-4 text-center col-span-full">No upcoming events scheduled.</p>
          ) : (
            events.map((e) => (
              <div key={e.id} className="p-4 rounded-2xl bg-cream/60 border border-beige/10">
                <div className="flex items-center gap-3 mb-2">
                  <div className="w-10 h-10 rounded-xl bg-pistachio/10 flex flex-col items-center justify-center shrink-0">
